...
export async function generateDashboard(dbPath?: string): Promise<string> {
  ...
  const dataScript = `
    <script>
      const DATA = ${JSON.stringify({ ...state, changes }, null, 2)};
      ...
      // Pipeline stats (from last radar run)
      document.getElementById('queries').textContent = '4';
      document.getElementById('hits').textContent = '13';
      document.getElementById('newUrls').textContent = '13';
      document.getElementById('candidates').textContent = '10';
